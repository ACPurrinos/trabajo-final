...
const FormCategoria = () => {
  ...
  const checkExistingCategory = async () => {
    try {
      const response = await fetch(`https://backtp-production.up.railway.app/categorias/check?nombre=${nombre}`, {
        method: 'GET',
      });
  ...
  const handleSubmit = async (event) => {
    ...
    if (confirmacion) {
      try {
        const response = await fetch('https://backtp-production.up.railway.app/categoria', {
          method: 'POST',
          headers: {
